...
// State untuk Ubah Cost (1)
const costMode = ref('percentage');
const costValue = ref(0);

// State untuk Ubah Price (2)
const priceReference = ref('cost'); // Default acuan ke "cost"
...
// Watcher untuk Filter (Terkirim otomatis saat user mengetik/memilih)
watch([search, category, brand], debounce((newValues) => {
    // Kita gunakan rute string langsung seperti di Index utama untuk menghindari error helper route()
    router.get('/produk/bulk-price', {
...
const selectAll = computed({
    ...
    set: (val) => {
        if (val) selectedIds.value = props.products.data.map(p => p.id);
...
const addToQueue = () => {
    ...
    router.post('/produk/bulk-queue', { ids: selectedIds.value }, {
        preserveScroll: true,
        onSuccess: () => {
            selectedIds.value = [];
...
// Helper kalkulasi preview Cost di tabel
const calculateCostPreview = (q: any) => {
    const currentCost = Number(q.cost || 0);
    const val = Number(costValue.value || 0);
    if (val === 0) return currentCost;

    if (costMode.value === 'percentage') {
        return currentCost + (currentCost * (val / 100));
    } else {
        return currentCost + val;
    }
};

// Helper kalkulasi preview Price di tabel (Reaktif terhadap Cost baru jika acuan = 'cost')
const calculatePricePreview = (q: any) => {
    ...
    // Tentukan nilai dasar berdasarkan dropdown acuan terpilih
...
// Modifikasi form submission ke backend
const processQueuePrice = () => {
    router.post('/produk/bulk-process-price', { 
        cost_mode: costMode.value,
        cost_value: costValue.value,
        ...
        preserveScroll: true,
        onSuccess: () => {
            if (props.queuedProducts.length <= 100) {
...
const removeFromQueue = (id) => {
    // 1. Debugging: Pastikan id adalah angka, bukan undefined
    console.log("Memproses lepas antrian untuk Produk ID:", id);

    if (!id) {
        alert("ID produk tidak terbaca!");
        return;
    }

    // 2. Eksekusi router.post
    router.post(`/produk/bulk-remove/${id}`, {}, {
        preserveScroll: true,
        onSuccess: () => {
            console.log("Berhasil dilepas dari antrian.");
        },
        onError: (errors) => {
            // Jika ada error validasi atau server, akan muncul di console
            console.error("Gagal memproses Inertia:", errors);
        }
    });
};
const removeFromQueueAll = () => {
    // 2. Eksekusi router.post
    router.post(`/produk/bulk-remove-all`, {}, {
        preserveScroll: true,
        onSuccess: () => {
            console.log("Berhasil dilepas dari antrian.");
        },
...
const sortedProducts = computed(() => {
    // Pastikan datanya ada, lalu gunakan spread pada .data
    if (!props.products.data) return [];
    
    return [...props.products.data].sort((a: any, b: any) => {
        
        // Fungsi untuk mengambil waktu log terakhir
        ...
        const timeA = getLatestLogTime(a);
        const timeB = getLatestLogTime(b);
...
const displayLatestEditTime = (product: any) => {
    // Cek apakah produk memiliki array act_log dan isinya tidak kosong
    if (product.act_log && product.act_log.length > 0) {
        // Ambil log urutan paling akhir
        const lastLog = product.act_log[product.act_log.length - 1];
        // Format datetime dari log tersebut menggunakan fungsi yang sudah Anda buat
        return formatDateTime(lastLog.datetime);
    }
    
    // Fallback: Jika produk belum punya log sama sekali, tampilkan updated_at
...
const formatDateTime = (dateString) => {
  ...
  // padStart digunakan agar bulan/tanggal/jam/menit selalu 2 digit (misal: 06, bukan 6)
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="py-6 sm:px-6 lg:px-8">
            ...
            <div class="mb-4 overflow-x-auto scroll-container pb-2 gap-3 flex mx-4 sm:mx-0">
                <input type="text" v-model="search" placeholder="Cari nama produk / SKU..." class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm text-sm">
                    ...
                    <table class="w-full text-left text-sm whitespace-nowrap">
                        <thead class="bg-gray-50 border-b border-gray-200 font-medium dark:*:bg-gray-700 dark:border-gray-600">
                            <tr>
                                ...
                                <th class="px-2 py-1.5">Nama Produk</th>
                                <th class="px-2 py-1.5">Kategori</th>
                                <th class="px-2 py-1.5">Brand</th>
                                ...
                                <th class="px-2 py-1.5 text-left">Terakhir ubah harga</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100">
                            <tr v-for="(item, index) in sortedProducts" :key="item.id" class="hover:bg-blue-100/50" :class="{'bg-yellow-50/50': item.queue_act}">
                                ...
                                <td class="px-2 py-1 text-gray-600 dark:text-gray-300">{{ item.categories?.map(c => c.name).join(', ') || '-' }}</td>
                                <td class="px-2 py-1 text-gray-600 dark:text-gray-300">{{ item.brand?.name || '-' }}</td>
                                ...
                                <td class="px-2 py-1 text-left text-xs">
                                    <button 
                                        v-if="item.act_log && item.act_log.length" 
                                        @click="openLogDialog(item)"
                                        class="inline-flex items-center px-2 py-1 bg-blue-50 text-blue-600 hover:bg-blue-100 hover:text-blue-800 rounded font-medium transition-colors"
                                    >
              ...
              >
                <div class="overflow-x-auto no-scrollbar">
                  <div class="flex w-max gap-1 lg:mx-auto">
                    ...
                    <!-- First -->
                    <button
                      :disabled="!props.products.first_page_url"
                      @click="router.get(props.products.first_page_url, {}, { preserveScroll: true })"
                    ...
                    <!-- Numbers -->
                    <button
                      v-for="(link, i) in props.products.links.filter(l => /^\d+$/.test(l.label))"
                      :key="i"
                    ...
                    <!-- Last -->
                    <button
                      :disabled="!props.products.last_page_url"
                      @click="router.get(props.products.last_page_url, {}, { preserveScroll: true })"
                      class="shrink-0 px-3 py-1 rounded-lg
                            bg-gray-200 dark:bg-gray-700
                            disabled:opacity-50"
                    >
        ...
        <div v-if="showLogDialog" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
            <div class="bg-background rounded-lg shadow-xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[80vh]">
                ...
                <div class="px-4 py-3 border-t bg-backgroundflex justify-end">
                    <button @click="closeLogDialog" class="px-5 py-2 text-sm bg-gray-200 text-gray-700 hover:bg-gray-300 rounded-md font-medium transition-colors">
                        Tutup
                    </button>
                </div>
        ...
        <div v-if="showDialog" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
            <div class="bg-background rounded-lg shadow-xl w-full max-w-3xl overflow-hidden flex flex-col max-h-[90vh]">
                <div class="px-4 py-3 border-b flex justify-between items-center bg-background ">
                    <div>
                        <h3 class="font-bold text-lg">Proses Antrian Harga</h3>
                        <p class="text-xs text-gray-500">Total ada {{ queuedProducts.length }} produk di antrian.</p>
                    </div>
                ...
                <div class="p-4 grid grid-cols-1 md:grid-cols-2 gap-4 border-b bg-accent/5">
                    ...
                    <div class="p-3 bg-background rounded border border-accent shadow-sm">
                        <h4 class="font-bold text-xs uppercase tracking-wider text-gray-700 mb-2 border-b pb-1 flex justify-between">
                            ...
                            <span v-if="Number(costValue) !== 0" class="text-[10px] text-blue-600 bg-blue-50 px-1 rounded font-normal">Aktif</span>
                        </h4>
                        <div class="flex gap-2 items-end">
                            <div class="w-1/2">
                                <label class="block text-[10px] text-gray-500 mb-0.5">Mode</label>
                                <select v-model="costMode" class="w-full text-xs border-gray-300 rounded focus:ring-blue-500 focus:border-blue-500 py-1 px-2">
                            ...
                            <div class="w-1/2">
                                ...
                                <input type="number" v-model="costValue" step="any" class="w-full text-xs border-gray-300 rounded focus:ring-blue-500 focus:border-blue-500 py-1 px-2">
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="grid grid-cols-3 gap-2 items-end">
                            <div>
                                ...
                                <select v-model="priceReference" class="w-full text-xs border-gray-300 rounded focus:ring-blue-500 focus:border-blue-500 py-1 px-1">
                                    <option value="cost">Cost (Modal)</option>
                                    <option value="price">Price (Jual)</option>
                                </select>
                            </div>
                            <div>
                                <label class="block text-[10px] text-gray-500 mb-0.5">Mode</label>
                ...
                <div v-if="queuedProducts.length > 100" class="bg-amber-50 text-amber-800 text-xs px-4 py-1.5 border-b border-amber-200">
                ...
                <div class="overflow-y-auto flex-1 p-4 bg-background">  
                    <table class="w-full text-xs border bg-background shadow-sm">
                        <thead class="bg-accent border-b sticky top-0 shadow-sm font-medium">
                            <tr>
                                <th class="px-2 py-1.5 text-left">Nama Produk</th>
                                <th class="px-2 py-1.5 text-right w-44">Cost Preview (Lama → Baru)</th>
                                <th class="px-2 py-1.5 text-right text-blue-600 w-44">Price Preview (Lama → Baru)</th>
                        ...
                        <tbody>
                            <tr v-for="(q, idx) in queuedProducts" :key="q.id" class="border-b last:border-0" :class="{'opacity-50 bg-gray-50': idx >= 100}">
                                ...
                                <td class="px-2 py-1 text-right">
                                    ...
                                    <span class="font-semibold text-gray-700" :class="{'text-blue-600 font-bold': Number(costValue) !== 0}">
                                ...
                                <td class="px-2 py-1 text-center">
                                    <button @click="removeFromQueue(q.id)" class="text-xs text-red-500 hover:text-red-700 font-medium hover:underline">
                                        Batal
                                    </button>
                                </td>
                            </tr>
                            <tr v-if="queuedProducts.length === 0">
                                <td colspan="4" class="px-2 py-4 text-center text-gray-500">Antrian kosong.</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                ...
                <div class="px-4 py-3 border-t bg-background flex justify-start items-center">
                    <button @click="removeFromQueueAll" class="text-xs mr-3 text-red-500 hover:text-red-700 font-medium hover:underline">
                        Hapus Semua
                    </button>                    
                    ...
                    <button @click="processQueuePrice" :disabled="queuedProducts.length === 0" class="px-4 py-2 text-sm bg-green-600 text-white rounded hover:bg-green-700 disabled:opacity-50 font-medium">
                        Eksekusi
                    </button>
                </div>
            </div>
...
<style scoped>
/* Menampilkan scrollbar horizontal yang elegan agar user bisa geser tabel */
.scroll-container {
...
.scroll-container::-webkit-scrollbar {
  height: 8px; /* Ketebalan scrollbar geser kiri-kanan */
  width: 6px;
}
